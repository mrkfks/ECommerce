    ...
    public async Task<IActionResult> GetAll([FromQuery] int? companyId = null)
    {
        try
        {
            if (companyId.HasValue)
            {
                ...
                return Ok(new ECommerce.Application.Responses.ApiResponse<IReadOnlyList<UserDto>>(true, "Kullanıcılar başarıyla alındı", users));
            }
            else
            {
        ...
        {
            return BadRequest(new ECommerce.Application.Responses.ApiResponse<IReadOnlyList<UserDto>>(false, $"Hata: {ex.Message}"));
        }
    }
